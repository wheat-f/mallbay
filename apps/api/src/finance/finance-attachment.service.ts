...
export class FinanceAttachmentService {
  ...
  async upload(actor: FinanceActor, applicationType: "EXPENSE" | "REIMBURSEMENT", applicationId: string, dto: UploadFinanceAttachmentDto, file: MulterFile) {
    ...
    const canViewOwn = await this.accessContext.can({ userId: actor.id }, FINANCE_CAPABILITIES.document.capability, FINANCE_CAPABILITIES.document.attach, { storeId: application.storeId, ownerId: application.applicantId });
    const canViewAll = await this.accessContext.can({ userId: actor.id }, FINANCE_CAPABILITIES.document.capability, FINANCE_CAPABILITIES.document.attach, { storeId: application.storeId });
