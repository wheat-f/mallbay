...
import { App, Button, Card, Checkbox, Drawer, Input, InputNumber, Select, Space, Table, Tag, Typography } from "antd";
...
export default function ConstructionCostSettlementsPage() {
  ...
  const [manualConstructionCharges, setManualConstructionCharges] = useState<Record<string, number>>({});
  const [useManualConstructionCharges, setUseManualConstructionCharges] = useState(false);
...
function CostSettlementDetail({ settlement, confirmedMinutes, confirmedReasons, timeVarianceReasonOptions, useManualConstructionCharges, manualConstructionCharges, onUseManualConstructionChargesChange, onManualConstructionChargeChange, onMinutesChange, onReasonChange }: { settlement: ConstructionCostSettlement; confirmedMinutes: Record<string, number>; confirmedReasons: Record<string, { code?: string; text?: string }>; timeVarianceReasonOptions: DictionaryOption[]; useManualConstructionCharges: boolean; manualConstructionCharges: Record<string, number>; onUseManualConstructionChargesChange: (checked: boolean) => void; onManualConstructionChargeChange: (workerUserId: string, amountYuan: number) => void; onMinutesChange: (workerUserId: string, minutes: number) => void; onReasonChange: (workerUserId: string, patch: { code?: string; text?: string }) => void }) {
  return <Space direction="vertical" size="large" style={{ width: "100%" }}>
    ...
    <Card size="small" title="施工收费分摊"><Checkbox checked={useManualConstructionCharges} onChange={(event) => onUseManualConstructionChargesChange(event.target.checked)}>店长手工分配施工金额</Checkbox><Typography.Paragraph type="secondary" style={{ margin: "8px 0" }}>默认按确认工时比例分摊；勾选后必须填写每位施工人员金额，合计必须等于本单施工收费。</Typography.Paragraph>{useManualConstructionCharges ? settlement.workerLines.map((line) => <div key={`charge-${line.workerUserId}`} style={{ marginBottom: 8 }}><Typography.Text>{line.worker?.realName ?? line.worker?.username ?? line.workerUserId}</Typography.Text><InputNumber min={0} precision={2} value={manualConstructionCharges[line.workerUserId]} onChange={(value) => onManualConstructionChargeChange(line.workerUserId, Number(value ?? 0))} addonBefore="¥" style={{ width: "100%", marginTop: 4 }} /></div>) : null}</Card>
...
function toDefaultConfirmation(settlement: ConstructionCostSettlement, confirmedMinutes: Record<string, number>, confirmedReasons: Record<string, { code?: string; text?: string }>, manualConstructionCharges: Record<string, number> = {}, useManualConstructionCharges = false): ConfirmCostSettlementPayload {
  return { workerLines: settlement.workerLines.map((line) => {
    ...
    const reason = confirmedReasons[line.workerUserId];
    return { workerUserId: line.workerUserId, confirmedMinutes: confirmed, ...(useManualConstructionCharges ? { manualConstructionChargeCents: Math.round((manualConstructionCharges[line.workerUserId] ?? 0) * 100) } : {}), ...(reason?.code ? { varianceReasonCode: reason.code } : {}), ...(reason?.text?.trim() ? { varianceReasonText: reason.text.trim() } : {}) };
  }) };
}
