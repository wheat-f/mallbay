...
export default function ExpenseListPage() {
  const storeId = useAuthStore((s) => s.user?.storeMember?.store.id); const position = useAuthStore((s) => s.user?.storeMember?.position); const router = useRouter(); const client = useQueryClient(); const { message } = App.useApp(); const [open, setOpen] = useState(false); const [form] = Form.useForm();
  const scope = position === "MANAGER" || position === "FINANCE" ? "all" : "mine"; const query = useQuery({ queryKey: ["finance-expenses", storeId, scope], queryFn: () => financeApi.expenses({ storeId: storeId!, scope }), enabled: Boolean(storeId) });
