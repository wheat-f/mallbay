...
const PATHS: Record<string, string> = {
  ...
  "store.profile": "/settings/store", "store.operations": "/settings/store", "store.notifications": "/settings/store", "store.capacity": "/settings/store",
...
export default function AllSettingsPage() {
  ...
  if (loading) return <div className="management-page"><Spin tip="正在加载全部设置…" /></div>;
  if (error) return <div className="management-page"><Alert type="error" showIcon message={error} /><Button onClick={() => router.push("/settings")}>返回职责工作台</Button></div>;
