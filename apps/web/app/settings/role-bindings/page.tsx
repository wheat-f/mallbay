...
export default function RoleBindingsPage() {
  ...
  const permissionsQuery = useQuery({ queryKey: ["auth-permissions", "headquarters"], queryFn: () => permissionsApi.me() });
  ...
  return (
    <div className="management-page">
      ...
      {isHeadquartersAdmin ? <Alert type="info" showIcon message="总部成员绑定暂未开放" description="当前总部管理员只能查询绑定状态；总部管理员初始化由权限迁移脚本完成，门店店长的门店成员管理不受影响。" /> : null}
      <Card title="选择人员">
      ...
      {!isHeadquartersAdmin ? <Card title="新增门店角色绑定" style={{ marginTop: 16 }}>
        <Space.Compact style={{ width: "100%" }}>
        ...
        {!userId ? <Empty description="请输入用户 ID" /> : (bindingsQuery.data ?? []).length === 0 ? <Empty description="暂无角色绑定" /> : (
          (bindingsQuery.data ?? []).map((binding) => (
            <div key={binding.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 0", borderBottom: "1px solid #f0f0f0" }}>
              ...
              <Space><Tag color={binding.status === "ACTIVE" ? "green" : "default"}>{binding.status === "ACTIVE" ? "生效中" : "已停用"}</Tag>{!isHeadquartersAdmin && binding.status === "ACTIVE" && <Button type="link" danger onClick={() => disableMutation.mutate(binding.id)}>停用</Button>}</Space>
            </div>
          ))
        )}
      </Card>
