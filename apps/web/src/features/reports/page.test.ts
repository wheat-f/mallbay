...
test("reports page converts report tabs to a visible mobile grid", () => {
  assert.match(cssSource, /@media \(max-width: 720px\)[\s\S]*\.reports-tabs\s*\{[\s\S]*display: grid;/);
  assert.match(cssSource, /@media \(max-width: 720px\)[\s\S]*\.reports-tabs\s*\{[\s\S]*grid-template-columns: repeat\(2, minmax\(0, 1fr\)\);/);
  assert.match(cssSource, /@media \(max-width: 720px\)[\s\S]*\.reports-tabs\s*\{[\s\S]*overflow-x: visible;/);
  assert.match(cssSource, /@media \(max-width: 720px\)[\s\S]*\.reports-tabs button\s*\{[\s\S]*white-space: normal;/);
});
