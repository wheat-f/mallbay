export function canAccessSystemSettings(input: {
  ...
  const storeCodes = new Set(["store.dictionary", "store.members", "store.profile", "store.operations", "store.notifications", "store.capacity", "settings.audit.store", "finance.labor_cost", "finance.settlement", "finance.accounts", "finance.audit"]);
