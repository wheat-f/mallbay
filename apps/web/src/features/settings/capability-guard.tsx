...
export function SettingsCapabilityGuard({ capabilityCodes, children }: Props) {
  ...
  if (state === "loading") return <div className="management-page"><Spin description="正在校验设置权限…" /></div>;
  if (state === "error") return <div className="management-page"><Alert type="error" showIcon title="设置权限校验失败" /><Button onClick={() => router.push("/settings")}>返回设置首页</Button></div>;
