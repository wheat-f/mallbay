...
- 通过 `HQ_ADMIN_USERNAME` 和 `HQ_ADMIN_PASSWORD` 初始化唯一总部管理员；
- 本地环境使用 `xiaoming`，测试库和生产使用 `zhouluoren`；
...
| T2 | 权限迁移脚本读取显式环境变量，创建/补齐/恢复单一总部管理员 | 已完成 | PR-001~006、AC-1~7、21~24、26~27 |
...
- `HQ_ADMIN_USERNAME` 必须显式配置；脚本不设置默认用户名；
- 目标用户不存在时才要求 `HQ_ADMIN_PASSWORD`，并使用现有 bcrypt 逻辑；
- 目标用户已存在时不重置密码；
...
1. 在本地开发环境显式配置 `HQ_ADMIN_USERNAME=xiaoming` 和本地 `HQ_ADMIN_PASSWORD`。
2. 在测试库、生产环境显式配置 `HQ_ADMIN_USERNAME=zhouluoren`，密码由密钥系统注入。
3. 执行迁移脚本并检查输出不含密码或密码哈希。
...
- `HQ_ADMIN_USERNAME` 不允许为空，不允许依赖脚本默认值；
- 本地必须是 `xiaoming`，测试库和生产必须是 `zhouluoren`；
- 目标用户已存在时不要因密码变量变化重置密码；
