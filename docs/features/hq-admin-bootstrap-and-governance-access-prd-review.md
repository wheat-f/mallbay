...
- 本地统一配置 `HQ_ADMIN_USERNAME=xiaoming`。
- 测试库、生产统一配置 `HQ_ADMIN_USERNAME=zhouluoren`。
- `HQ_ADMIN_PASSWORD` 只通过环境变量/密钥注入，创建新用户时使用，既有用户不重置密码。
- 迁移脚本不猜测环境、不使用隐式默认账号；部署预检负责环境账号映射。
- 用户创建、绑定补齐/恢复、审计写入在同一事务内完成，失败回滚。
...
1. 环境变量模板、CI/CD 或启动脚本需要显式提供 `HQ_ADMIN_USERNAME`，并在部署预检中校验环境映射。
2. 并发初始化需要在事务内重新检查有效 `HQ_ADMIN/HQ` 绑定，确保最终最多一个有效绑定。
3. `HQ_ADMIN_PASSWORD` 不得出现在日志、异常、审计 metadata、前端 bundle 或提交内容中。
