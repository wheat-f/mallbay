...
- 个人资料：昵称、头像、密码、邮箱/手机/微信/支付宝绑定、审核员重置密码。
- 门店管理：审核员创建门店、指派店长、门店列表、门店详情、冻结/解冻、店长变更。
- 门店成员：店长邀请成员、成员接受/拒绝邀请、移除成员、岗位枚举。
- 门店送审：店长编辑门店资料和照片、提交审核、审核员通过/驳回。
...
- `InventoryMovement` MUST 关联产品批次和业务来源，支持订单出库、采购入库、盘点、报损、调拨。
...
- `InventoryBatch`、`InventoryMovement`、`PurchaseOrder`、`PurchaseOrderItem`。
...
- 小程序离线同步存在冲突处理和上传失败重试问题，不适合作为第一阶段。
- 当前岗位枚举不能完整表达客服、老板、管理员，需要在权限策略中预留扩展点。
