...
- 售后任务作用域：销售只能查看本人订单对应售后单，师傅和学徒只能查看指派给自己的售后单。
...
- 销售：只能查看本人订单对应售后单。
- 财务：管理本门店提成规则和提成生成。
- 师傅/学徒：只能查看指派给自己的售后单，当前阶段不直接处理售后单写操作。
...
- MUST 对销售访问售后列表时按 `order.salesPersonId` 过滤，不允许销售查看同门店其他销售订单的售后单。
- MUST 通过提成 API 生成提成日志，不允许前端直接写提成表。
- MUST 保留订单、施工记录、售后、处罚和提成的追溯关系。
- MUST 在售后派工、处罚人员和提成调整人员选择器中展示施工人员业务标签；师傅提成的施工记录选择器必须展示施工状态中文业务标签；提成规则类型必须展示为中文业务文案，不得直接展示枚举值。
