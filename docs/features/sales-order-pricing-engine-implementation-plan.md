...
> 后续口径调整：本文记录首版建议价与审批能力；施工收费、标准工时、预计/实际成本和完工结算的语义拆分及后续实施，以[销售订单施工收费与成本核算调整实施计划](./sales-order-construction-charge-cost-implementation-plan.md)为准。
...
- API 单元回归：302 项通过；覆盖订单、容量、价格引擎、报价、产品、库存、权限、审计持久化和既有业务模块；新增覆盖规则草稿详情/更新、车型价格级别与映射更新、报价详情、报价草稿提交、当前已发布规则自动匹配、已审批报价转单和重复转单幂等。
- Web 回归：584 项通过；新增规则版本详情、报价详情、草稿提交操作和相应页面契约覆盖，并保留草稿价格快照恢复选择覆盖。
...
- 计划契约补齐：规则版本 GET/PATCH、车型价格级别 PATCH、车型映射 PATCH、报价详情 GET、报价草稿提交 POST 均已实现；规则版本详情页和报价详情页已加入主列表入口。
- 最新 API 真实认证验收：规则详情返回 PUBLISHED v2，已发布版本 PATCH 被服务端拒绝；报价详情返回 200；报价创建为 DRAFT、提交后进入 PENDING_APPROVAL、撤回后进入 WITHDRAWN。验收过程中发现并修复报价详情误选已删除 Customer.phone 字段导致的 500，并增加查询字段回归断言。
